...
class Messages {
  ...
  Create(title, content, csv, groupID, commitby, callback) {
    const query = `call InsertMessages("${escape(title)}","${escape(
      content
    )}","${escape(csv)}","${escape("[]")}",${groupID},"${commitby}")`;
  ...
  Select(ID, callback) {
    const query = `select ID,GroupID,Title,Content,Data,SentData from Messages where ${
      ID ? ` ID=${ID} AND` : ""
    } DELETED = 0`;
    con.query(query, (err, result, fields) => {
      if (err) {
  ...
  GetGroup(ID = null, callback) {
    const query = `SELECT * FROM MessageGroups WHERE ${
      ID ? `ID=${ID} AND` : ``
    } Deleted=0`;
    console.log(query);
    con.query(query, (err, result, fields) => {
