...
class Templates {
  ...
  Save(title, content, groupid, commitby, callback) {
    const query = `call InsertTemplate("${escape(title)}","${escape(
      content
    )}",${groupid},"${commitby}");`;
  ...
  Update(ID, content, commitby, callback) {
    const query = `UPDATE Templates SET Content="${escape(
      content
    )}", UpdatedBy=${commitby},UpdatedDate=NOW() where ID="${ID}" and Deleted=0`;
    con.query(query, (err, result, fields) => {
      if (err) {
        callback(err);
      } else {
  ...
  GetGroup(ID = null, callback) {
    const query = `SELECT * FROM TemplateGroups WHERE ${
      ID ? `ID=${ID} AND` : ``
    } Deleted=0`;
    con.query(query, (err, result, fields) => {
      if (err) {
