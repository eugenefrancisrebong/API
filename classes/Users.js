...
class Users {
  ...
  Search(callback) {
    // let result;
    con.query(
      `SELECT ID,Username,Email,Firstname,Lastname,Designation,PermissionLevel,CreatedBy,UpdatedBy,CreatedDate,UpdatedDate FROM users where Deleted="0" `,
      (err, result, fields) => {
        if (err) callback(err);
  ...
  SearchOne(ID, callback) {
    // let result;
    con.query(
      `SELECT ID,Username,Email,Firstname,Lastname,Designation,PermissionLevel,CreatedBy,UpdatedBy,CreatedDate,UpdatedDate FROM users where Deleted="0" and ID="${ID}"`,
      (err, result, fields) => {
        if (err) callback(err);
        return callback(result);
      }
